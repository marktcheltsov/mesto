...
function getCard(cardsLink, cardsName, templateSelector) {
  ...
  function prependCard(cardsLink, cardsName, templateSelector) {

  }

 function creatDefoltCard() {
  for (let i = 0; i < initialCards.length; i++) {
    const card = getCard(initialCards[i].link, initialCards[i].name, '.template');
...
function creatNewCard(evt) {
  evt.preventDefault();
  const card = getCard(inputCardsLink.value, inputCardsName.value, '.template');
  cardsContainer.prepend(card);
